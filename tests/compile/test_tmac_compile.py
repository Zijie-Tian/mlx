...
if __name__ == "__main__":
    generate_stream = mx.new_stream(mx.cpu)
    ...
    def measure_latency(layer, input_tensor, warmup=10, runs=100):
        # Warmup iterations
        for _ in range(warmup):
            layer(input_tensor)
        # Timing iterations using high-precision timer
        start = time.perf_counter()
        for _ in range(runs):
            layer(input_tensor)
        end = time.perf_counter()
        return ((end - start) / runs) * 1e6  # return average latency in milliseconds

    # 测试编译前的延迟
    pre_latency = measure_latency(ql, activation)
    print("Pre-compiled average latency: {:.6f} msec".format(pre_latency))

    # 编译 ql 层，假设 compile() 方法返回一个编译后的对象
    compiled_ql = mx.compile(ql)

    # 测试编译后的延迟
    post_latency = measure_latency(compiled_ql, activation)
    print("Post-compiled average latency: {:.6f} msec".format(post_latency))
