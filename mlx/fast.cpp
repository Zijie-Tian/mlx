...
std::vector<array> HermesDecode::vjp(
  const std::vector<array>& primals,
  const std::vector<array>& cotangents,
  const std::vector<int>& argnums,
  const std::vector<array>&) {
...
std::vector<array> HermesDecode::jvp(
  const std::vector<array>& primals,
  const std::vector<array>& tangents,
  const std::vector<int>& argnums) {
  if (argnums.size() > 1 || argnums[0] != 0) {
    throw std::runtime_error(
        "[HermesDecode::jvp] No JVP wrt the quantized matrix yet.");
  }
  return {tangents[0]};
}

std::pair<std::vector<array>, std::vector<int>> HermesDecode::vmap(
  const std::vector<array>& inputs,
  const std::vector<int>& axes) {
  return {};
}

std::vector<Shape> HermesDecode::output_shapes(
    const std::vector<array>& inputs) {
  return {};
}
