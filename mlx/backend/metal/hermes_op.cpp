...
TVMRuntime* HermesDecode::_tvm_internals = nullptr;
#endif
INIReader* Hermes::_reader       = nullptr;
INIReader* HermesDecode::_reader = nullptr;

ThreadPool Hermes::cpu_thread_pool{12};
ThreadPool Hermes::gpu_thread_pool{4};
ThreadPool HermesDecode::cpu_thread_pool{12};
ThreadPool HermesDecode::gpu_thread_pool{4};
...
#else
            LOG(FATAL)
                << "Please set TMAC_KERNELS_LIBRARY environment variable";
            return "";
#endif
        }
    }
...
#if defined(USE_TVM_THREADPOOL) || defined(USE_TVM_LIB)
tvm::runtime::PackedFunc get_function(TVMRuntime* _tvm_internals,
                                      std::mutex& _m,
                                      const std::string& func_name, _fkey key)
{
...
void launch_qmm_high(std::string name, const std::vector<array>& inputs,
                     array& out, int group_size, int bits, int D, int O, int B,
                     int N, MTL::Size& group_dims, MTL::Size& grid_dims,
                     bool batched, bool matrix, bool gather, bool aligned,
                     bool quad, const Stream& s)
{
    auto& x_pre      = inputs[0];
    auto& w_pre      = inputs[1];
    ...
    if (quad)
    {
        template_def = get_template_definition(kname.str(), name, type_string,
                                               group_size, bits, D, batched);
    }
    else if (aligned && !gather)
    {
        template_def =
            get_template_definition(kname.str(), name, type_string, group_size,
                                    bits, aligned_n, batched);
    }
    else if (!gather && !aligned)
    {
        template_def = get_template_definition(kname.str(), name, type_string,
                                               group_size, bits, batched);
    }
    else if (aligned && gather)
    {
        template_def = get_template_definition(kname.str(), name, type_string,
                                               group_size, bits, aligned_n);
    }
    else
    {
        template_def = get_template_definition(kname.str(), name, type_string,
...
void qvm_high_split_k(const std::vector<array>& inputs, array& out,
                      int group_size, int bits, int D, int O, int B, int N,
                      const Stream& s)
{
    ...
    std::ostringstream kname;
    auto type_string = get_type_string(x.dtype());
    kname << "qvm_split_k" << "_" << type_string << "_gs_" << group_size
          << "_b_" << bits << "_spk_" << split_k;
    ...
    strided_reduce_general_dispatch(intermediate, out, "sum", plan, {axis},
                                    compute_encoder, d, s);
}

void qmm_op_high(const std::vector<array>& inputs, array& out, bool transpose,
                 int group_size, int bits, bool gather, const Stream& s)
{
    ...
    else
    {
        ...
        {
            return qvm_high_split_k(inputs, out, group_size, bits, D, O, B, N,
                                    s);
        }
        else if (B < 4)
        {
        ...
        else
        {
            ...
            if ((O % bn) != 0)
            {
                ...
                msg << "[quantized_matmul] The output size should be divisible "
                       "by "
                    << bn << " but received " << O << ".";
    ...
    launch_qmm_high(name, inputs, out, group_size, bits, D, O, B, N, group_dims,
                    grid_dims, batched, matrix, gather, aligned, quad, s);
}
...
#endif
               + "_int8" + "_m" +
               std::to_string(std::get<0>(key) * std::get<3>(key)) + "_k" +
               std::to_string(std::get<1>(key)) + "_n" +
               std::to_string(std::get<2>(key)) + "_b" +
               std::to_string(std::get<3>(key));
    }
    else
    {
        return std::string("preprocessor")
#if defined(USE_TVM_LIB) && !defined(USE_TVM_THREADPOOL)
               + "_t" + std::to_string(1)
#else
               + "_t" + std::to_string(_n_threads)
...
HermesConfig Hermes::get_kcfg(int M, int K, int N, int bits)
{
    ...
    return {
        ...
        /* .simd_n_in       = */
        (int)_reader->GetInteger(section, "simd_n_in", 0),
        /* .simd_n_out      = */
        (int)_reader->GetInteger(section, "simd_n_out", 0),
        /* .kfactor         = */
        (int)_reader->GetInteger(section, "kfactor", 0),
        /* .group_size      = */
        (int)_reader->GetInteger(section, "group_size", 0),
        /* .lut_scales_size = */
        (int)_reader->GetInteger(section, "lut_scales_size", 0),
        /* .scales_size     = */
        (int)_reader->GetInteger(section, "scales_size", 0),
        /* .n_tile_num      = */
        (int)_reader->GetInteger(section, "n_tile_num", 0),
    };
}

Hermes::Hermes(Stream stream, const std::string& kcfg_file,
               const std::string& library_file, int M_high, int M_low, int K,
               int N, bool transpose_high, int group_size_high, int nbits_high,
               int nbits_low)
    : UnaryPrimitive(stream),
      ...
      N_(N),
      nbits_low(nbits_low),
      transpose_high(transpose_high),
      group_size_high(group_size_high),
      nbits_high(nbits_high),
      _n_threads(12)
{
    ...
    {
        LOG(FATAL)
            << "Cannot find the configuration for the given TMAC parameters"
            << " (M, K, N, nbits) = (" << M_low << ", " << K_ << ", "
            << this->N_low_kernel << ", " << nbits_low << ")";
    }
    ...
    if (Hermes::_tvm_internals == nullptr)
    {
        ...
        _tvm_internals->_mod_lib =
            (*tvm::runtime::Registry::Get("runtime.SystemLib"))();
#else
        LOG(INFO) << "Loading kernels from: " << get_library_file(library_file);
        _tvm_internals->_mod_lib =
            tvm::runtime::Module::LoadFromFile(get_library_file(library_file));
#endif

#ifdef USE_TVM_THREADPOOL
        _tvm_internals->_config_threadpool =
            tvm::runtime::Registry::Get("runtime.config_threadpool");
        set_num_threads(_n_threads);
#else

#endif
        _tvm_internals->pf =
            get_function(Hermes::_tvm_internals, this->_m,
...
#else
                         this->get_template_name(
                             {bm_, K_, this->N_low_kernel, nbits_low, 1}),
#endif
                         {M_low, K_, this->N_low_kernel, nbits_low, 1});
    }
#endif
...
#if defined(_WIN32)
    _qlut       = _aligned_malloc(maxN * maxK / g_ * (1 << g_) * sizeof(int8_t),
                                  kAllocAlignment);
    _lut_scales = _aligned_malloc(
        maxN * maxK / act_group_size_ * sizeof(float16_t), kAllocAlignment);
    _lut_biases = _aligned_malloc(
        maxN * maxK / act_group_size_ * sizeof(float16_t), kAllocAlignment);
#else
    posix_memalign(&_qlut, kAllocAlignment,
                   maxN * maxK / g_ * (1 << g_) * sizeof(int8_t));
    posix_memalign(&_lut_scales, kAllocAlignment,
                   maxN * maxK / act_group_size_ * sizeof(float16_t));
    posix_memalign(&_lut_biases, kAllocAlignment,
                   maxN * maxK / act_group_size_ * sizeof(float16_t));
#endif
    _allocated = true;
}
...
void Hermes::set_num_threads(int n_threads)
{
    ...
    if (n_threads != _n_threads)
    {
        ...
        int num_threads =
            (*tvm::runtime::Registry::Get("runtime.NumThreads"))();
...
void Hermes::eval_gpu(const std::vector<array>& inputs, array& out)
{
    ...
    // out_low.copy_shared_buffer(out, {0}, out.flags(), out_low.nbytes(), N_ *
    // M_high);
    ...
    // qmm_op_high(inputs, out_high, transpose_high, group_size_high,
    // nbits_high, false, stream());
    ...
    std::future<int> future = Hermes::gpu_thread_pool.enqueue(
        [=]() mutable
        {
    ...
    int64_t activation_bn_shape[2] = {
        1, K_};  // TODO : Now we only support batch size = 1.
    ...
    DLTensor B              = {(void*)activations_buf, cpu_dev, 2, float_dtype,
                               activations_shape};
    DLTensor A      = {(void*)qweight_buf, cpu_dev, 3, int_dtype, A_shape};
    DLTensor Scales = {(void*)scales_buf, cpu_dev, 3, float_dtype,
                       Scales_shape};
    ...
    DLTensor LUTSt = {(void*)(this->_lut_scales), cpu_dev, 2, float_dtype,
                      luts_shape};
    DLTensor LUTBt = {(void*)(this->_lut_biases), cpu_dev, 2, float_dtype,
                      luts_shape};
    ...
    {
        for (int m_tile_idx = 0;
             m_tile_idx < this->M_low / (this->bm_ / ngroups_per_elem);
             m_tile_idx++)
        {
            DLTensor C_tile = {
    ...
    {
        DLTensor B_row = {/* .data   = */ (void*)(activations_buf + n_idx * K_),
                          /* .device = */ cpu_dev,
                          /* .ndim   = */ 2,
                          /* .dtype  = */ float_dtype,
        ...
        for (int m_tile_idx = 0; m_tile_idx < M_low / bm_; m_tile_idx++)
        {
            tiles.emplace_back(Hermes::cpu_thread_pool.enqueue(
                [this, &A, &QLUTt, &Scales, &LUTSt, &LUTBt, &C_tiles,
                 m_tile_idx, n_idx, ngroups_per_elem]() -> int
                {
                    // std::cout << "Hermes::eval_gpu: Start low precision GEMM
                    // on CPU stream." << std::endl;
                    (_tvm_internals->qf)(
                        &A, &QLUTt, &Scales, &LUTSt, &LUTBt,
                        &C_tiles[m_tile_idx +
                                 n_idx * this->M_low /
                                     (this->bm_ / ngroups_per_elem)]);
                    // std::cout << "Hermes::eval_gpu: Finish low precision GEMM
                    // on CPU stream." << std::endl;
                    return 0;
                }));
        }
...
std::string HermesDecode::get_template_name(_fkey key)
{
    if (std::get<4>(key) != 0)
    {
        return std::string("qgemm_lut")
...
HermesConfig HermesDecode::get_kcfg(int M, int K, int N, int bits)
{
    // TODO: find a better way to find kcfg when _n_threads is unknown
    const std::vector<int> n_threads_hints = {1, 4, 8, 12, 16};
    std::string section;
...
HermesDecode::HermesDecode(Stream stream, const std::string& kcfg_file,
                           const std::string& library_file, int M_high,
                           int M_low, int K, int N, bool transpose_high,
                           int group_size_high, int nbits_high, int nbits_low)
    : UnaryPrimitive(stream),
      M_high(M_high),
      M_low(M_low),
      K_(K),
      ...
      _n_threads(12)
{
    if (HermesDecode::_reader == nullptr)
    {
        HermesDecode::_reader = new INIReader(get_kcfg_file(kcfg_file));
    }
...
#ifdef USE_TVM_LIB
    if (HermesDecode::_tvm_internals == nullptr)
    {
        HermesDecode::_tvm_internals = new TVMRuntime();
#ifdef TMAC_USE_SYSLIB
        _tvm_internals->_mod_lib =
            (*tvm::runtime::Registry::Get("runtime.SystemLib"))();
#else
...
#endif
        _tvm_internals->pf =
            get_function(HermesDecode::_tvm_internals, this->_m,
                         this->get_template_name(
                             {M_low, K_, this->N_low_kernel, nbits_low, 0}),
                         {M_low, K_, this->N_low_kernel, nbits_low, 0});
        _tvm_internals->qf =
...
// Should only be called in main thread
void HermesDecode::set_workspace(int maxM, int maxK, int maxN)
{
    int ngroups_per_elem = 8 / g_;
#if defined(_WIN32)
    _qlut       = _aligned_malloc(maxN * maxK / g_ * (1 << g_) * sizeof(int8_t),
...
void HermesDecode::set_num_threads(int n_threads)
{
#ifdef USE_TVM_THREADPOOL
    if (n_threads != _n_threads)
    {
...
HermesDecode::~HermesDecode()
{
    // Destructor logic if needed
}

void HermesDecode::eval_gpu(const std::vector<array>& inputs, array& out)
{
    // Pre-Allocate output buffer.
    out.set_data(allocator::malloc_or_wait(out.nbytes()));
    ...
    std::future<int> future = HermesDecode::gpu_thread_pool.enqueue(
        [=]() mutable
        {
            //> Start the high precision GEMM on GPU stream.
            qmm_op_high(inputs, out_high, transpose_high, group_size_high,
    ...
    for (int n_idx = 0; n_idx < N_tiles; n_idx++)
    {
        ...
        for (int m_tile_idx = 0; m_tile_idx < M_low / bm_; m_tile_idx++)
        {
            tiles.emplace_back(HermesDecode::cpu_thread_pool.enqueue(
                [this, &A, &QLUTt, &Scales, &LUTSt, &LUTBt, &C_tiles,
                 m_tile_idx, n_idx, ngroups_per_elem]() -> int
                {
                    (_tvm_internals->qf)(
