...
def compile(
    ...
    def make_call(kernel_def: str) -> bool:
        kernel_def_ptn = r"int32_t (\w+)_t(\d+)_int8_m(\d+)_k(\d+)_n(\d+)_b(\d+)"
        match = re.search(kernel_def_ptn, kernel_def)
        if not match:
            raise RuntimeError("Wrong kernel definition: {}".format(kernel_def))
        name, num_threads, m, k, n, b = match[1], match[2], match[3], match[4], match[5], match[6]
        ...
        wrapper_func_call = """
    if (m == {m} && k == {k} && n == {n} && b == {b}) return {name}_t{num_threads}_int8_m{m}_k{k}_n{n}_b{b}({wrapper_func_call});
""".format(m=m, k=k, n=n, b=b, name=name, num_threads=num_threads, wrapper_func_call=wrapper_func_call)
    ...
    def make_call_from_IRModule(mod: tvm.IRModule) -> bool:
        for gvar, func in mod.functions.items():
            ...
            if match:
                ...
                # 生成函数声明（假设返回类型为 int32_t）
                signature = get_func_signature(func)  # 假设这是你的参数类型解析函数
                args = ", ".join([f"{dtype} {name}" for name, dtype in signature])
                header = f"int32_t {func_name}({args});"
                
                wrapper_func_defs[op_name] = "inline int {}_int8(int m, int k, int n, int b, {})".format(
                    op_name,
                    ", ".join(f"{dtype} {name}" for name, dtype in signature)
                )
                wrapper_func_call = ", ".join(name for name, dtype in signature)
                wrapper_func_call = """
    if (m == {m} && k == {k} && n == {n} && b == {b}) return {name}_t{num_threads}_int8_m{m}_k{k}_n{n}_b{b}({wrapper_func_call});
""".format(m=m_val, k=k_val, n=n_val, b=b_val, name=op_name, num_threads=num_threads, wrapper_func_call=wrapper_func_call)
                wrapper_func_calls[op_name] += wrapper_func_call

            # import pdb; pdb.set_trace()

        return False

    def insert(all: Union[tvm.IRModule, Tuple[str]], new: Union[tvm.IRModule, Tuple[str]], force_update=False):
        if isinstance(new, tuple):
            # NOTE: ONLY C code generate can enter following branch.
            #! This branch the `new` is a tuple of (header, body)
            if make_call(new[0]):
                return all
        elif isinstance(new, tvm.IRModule):
            make_call_from_IRModule(new)
        if all is None:
            return new
    ...
    for bits, M, K, N, m_groups in kernel_shapes:
        ...
        mod = insert(mod, qgemm_mod, force_update=True)
        # Write kcfg
    ...
    def header_declare_from_IR(mod: tvm.IRModule) -> str:
        extern_C_str = "#ifdef __cplusplus\nextern \"C\" \n#endif\n"
        header = ""
        for gvar, func in mod.functions.items():
            func_name = gvar.name_hint
            signature = get_func_signature(func)
            args = ", ".join([f"{dtype} {name}" for name, dtype in signature])
            header += f"{extern_C_str} int32_t {func_name}({args});\n"
        return header
    ...
    if return_type == "lower":
        ...
        with open(os.path.join(FLAGS.out_path, "kernels.h"), "w") as f:
            ...
            f.write('#include "stdint.h"\n' + header_declare_from_IR(mod) + wrapper_func)
    else:
        with open(os.path.join(FLAGS.out_path, "kernels.h"), "w") as f:
            wrapper_func = (wrapper_func_defs["qgemm_lut"] + " {\n" + wrapper_func_calls["qgemm_lut"] + "\n    return -1;\n}\n" +
                            wrapper_func_defs["preprocessor"] + " {\n" + wrapper_func_calls["preprocessor"] + "\n    return -1;\n}\n")
