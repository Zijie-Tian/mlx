...
if __name__ == "__main__":
    ...
    for _ in range(num_tests):
        ...
        w_np = np.random.randint(0, 2**bits, size=(M, K), dtype="uint8")
        w_mlx = mx.array(w_np) #! Default is copy.

        # Generate scales
        scales_np = np.random.randn(M, K // g).astype(np.float32)
        scales_mlx = mx.array(scales_np)

        # Generate zeros
        zeros_np = np.random.randn(M, K // g).astype(np.float32)
        zeros_mlx = mx.array(zeros_np)

        # Test np_preprocess_weights
        w_permuted_np, scales_permuted_np = np_preprocess_weights(
            w_np, scales_np, zeros_np, bits, g, bm, kfactor, simd_n_in, simd_n_out
        )

        # Test mlx_preprocess_weights
        w_permuted_mlx, scales_permuted_mlx = mlx_preprocess_weights(
            w_mlx, scales_mlx, zeros_mlx, bits, g, bm, kfactor, simd_n_in, simd_n_out
        )

        # Check if the outputs are the same
        w_are_equal = np.allclose(w_permuted_np, w_permuted_mlx)
        scales_are_equal = np.allclose(scales_permuted_np, scales_permuted_mlx)

        all_w_equal = all_w_equal and w_are_equal
        all_scales_equal = all_scales_equal and scales_are_equal

    print(f"Are all permuted weights equal in 100 tests? {all_w_equal}")
    print(f"Are all permuted scales equal in 100 tests? {all_scales_equal}")
